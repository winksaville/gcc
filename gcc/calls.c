  ...
  if (arg->value == arg->stack)
  ...
  else if (arg->mode != BLKmode)
    {
	 ...
	 will prevent such mismatches.  */
      ...
      if (RECORD_OR_UNION_TYPE_P (TREE_TYPE (pval))
	  && TYPE_EMPTY_RECORD (TREE_TYPE (pval)))
	size = 0;
      else
      ...
      else
	{
	  ...
	  bool empty_record = (RECORD_OR_UNION_TYPE_P (TREE_TYPE (pval))
			       && TYPE_EMPTY_RECORD (TREE_TYPE (pval)));
...
must_pass_in_stack_var_size_or_pad (machine_mode mode, const_tree type)
{
  ...
  if (RECORD_OR_UNION_TYPE_P (type) && TYPE_EMPTY_RECORD (type))
    return false;
